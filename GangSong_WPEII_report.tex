...
When $\w$ is close to $0$, one only checks the derivative $\p_{\w}\exp(\w)$ at $0$, similar to the small deformation methods. Arsigny et al showed in \cite{Arsigny2006} that:
\begin{align}
...
Although the exponential of velocity fields not forming a strict Lie group, Bossa et al has shown the Baker-Campbell-Hausdorff formula for Lie group could be applied successfully. In the settings of LogDemons, Vercauteren used the following first order approximation:
\begin{align}
