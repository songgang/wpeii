...
Diffeomorphism transform is a nice mathematical model for large deformation image registration problems. We mainly discussed four types of parameterization models used in the diffeomorphism registration: Beg's LDDMM , Ashburner's DARTEL, Vercauteren's diffeomorphic demons and LogDemons. A unifed framework was first established from the examples of small deformation methods, which as we show serve as the base algorithms for their large deformation counterparts. By deriving the smoothing operation on the transform parameters from the regularization terms, we generalized the common framework as the optimization in the Sobolev space. Next, we discuss the differences between parameterization used in the four methods and their optimization strategies.  We compare the relationship between these algorithms and
discuss how they can be converted to each other by simplifying
assumptions or changing optimization scheme etc. We show that these
...
Image registration is an important medical image anaylysis technique. It generates a mapping or a transform between two images. Such a mapping is essential for many medical applications. One can construct an anatomy template by registering a group of images. Various statistics can be defined after images are registered to the same template. Also the mapping is userful to build the motion model from pulmonary and cardiac images. It also helps to discover the longitudinal change and quantify the effect of surgery.

Image registration is always formulated as an optimization problem. An image similarity funcition defines how two images are called ``similar'' after registration. However, this is an ill-posed problem. There are inifinite ways to warp one image into the other. We normally assume the optimal transform should be smooth, which is controlled by the regularization term in the energy funciton. 

Different parameterization of transforms leads to different regularizations. Historically, transform with small deformation was first studied as an optical flow problem \cite{Horn1981}. And later Thirion proposed a his demons algorithm \cite{Thirion98}, which was a heurisic approach without solid theorectical root by the time. Given its pratical success, there were theoretical efforts \cite{Cachier2003} to justify the demons algorithm later. Small deformation parameterize the transform by the displacement field of the transform, which is analogous to the elasticity of the spring. The performance of these models usually degrades rapidly for images with large deformations. One important reason is that small deformation models do not preserve the topology of the image structure. In other words, there is no quanrantee about the one-to-one mapping property for the existense of the inverse transform.

Large deformation methods aim to solve the problem. The notion of diffeomorphism was adopted in image registration. A diffeomorphism is a differentiable map with a differentiable
inverse, which form a group structure. It has a solid mathmatical background. Unlike small deformation, large deformation methods solves the whole path of transforming one image to the other. The transform model was no longer defined on the end point of the transform, but on the entire spatial-temporal domain. This is analogous to track the motion of the fluid. 

Trouv\'{e} gave some theorectical analysis about the diffeomorphism group \cite{Trouve1998}. Beg et al dervied a rigorous model \cite{Beg2005Computing} using the velocity fields as parameterization. To simplify the computation, Ashburner \cite{Ashburner2007} proposed to use a stationary velocity field to approxiamte the time-vayring velocity field. Especially he introduced a fast numerical method to compute the derivative in the optimization step. Building on Arsigny' Log-Euclidean framework \cite{Arsigny2006}, Vercauteren adopted a similar idea and extended the classical demons into the diffeomorphism transform \cite{Vercauteren2008Symmetric, Vercauteren2009}, which are efficient in computation. Diffeomorphism has been proven to be an effective way to compute large deformation image registration. Later, this approach is formulated more rigoriously \cite{Vercauteren2008Symmetric}, which works completely in the log-domain. Recent improvement by Mansi \cite{Mansi2010} extended the deformation with incompressibility constraints, i.e. the divergence free. Also, interesting work \cite{Chen2010} from Chen independently proposed another more flexible way to control divergence as well as the curl of the deformation field.


One way to understanding these approaches is to study the different forms of parameterization used for the diffeomorphism model. In this paper, we focus on four popular models: LDDMM from Beg et al  \cite{Beg2005Computing},
...
This report will illustrate several popular algorithms for
diffeomorphic image mapping. Our discussion follows as two steps. We first try to unify the general image registration in the same framework, using the small deformation as examples. We discuss various regularization approaches applied in these
...
We especially fill in the mathematical details, assuming a
common engineering background for readers, to aid in a complete understanding of
the derivation of the most important equations. Many techniques commonly used in diffeomorphism have root in the derivation of the small deformation models and the regularization terms. 
